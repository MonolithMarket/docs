...
Monolith runs a live bug bounty program hosted on the [Sherlock](https://sherlock.xyz) platform. Security researchers who responsibly disclose valid vulnerabilities in Monolith's smart contracts are eligible for rewards based on severity. All submissions must be made through the Sherlock platform in accordance with their [platform rules](https://docs.sherlock.xyz/bug-bounties/post-launch-bounty/platform-rules).
...
Reward amounts within ranges are determined at the discretion of the Monolith team based on the actual impact, exploitability, and quality of the report. Payment is made in accordance with Sherlock's standard payout process.
