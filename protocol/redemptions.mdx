...
- **Per-borrower redemption**
  ...
  - Result: Total free debt becomes 900. Bob’s debt drops to 300 (−100) while Alice's remains unchanged. Collateral of equal value is seized only from Bob to fund the redeemer.

- **Effect of the redemption fee**
  - Setup: Same as above, with a 0.3% redemption fee.
  - Redemption: User redeems 100 `Coin` from Bob; The user receives slightly less collateral than the fair price according to the oracle.
...
- Borrowers in free mode: You pay no interest, but a holder can redeem against your specific position and seize your collateral while repaying your debt. Other free-debt borrowers are not haircut. Your debt falls only when you are the chosen borrower.
