...
- Price gate: Write-offs require `allowLiquidations` from `getCollateralPrice()`. That fails only when the oracle feed reverts or returns a non-positive price. Staleness alone does not disable write-offs (it enables reduce-only and decays price).
