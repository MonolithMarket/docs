...
A position becomes liquidatable when borrowing power < debt at the current oracle price, where borrowing power = price × collateral × collateralFactor.

Liquidations require `allowLiquidations` from `getCollateralPrice()`. That flag is `false` only when the oracle feed reverts or returns a non-positive price. A stale feed does **not** disable liquidations: it enables reduce-only mode and decays the reported price over `STALENESS_UNWIND_DURATION` (24 hours) instead.
...
- Oracle-gated: If the oracle feed reverts or returns a non-positive price, liquidations are disabled. Stale prices keep liquidations enabled but switch the market to reduce-only and apply price decay.
