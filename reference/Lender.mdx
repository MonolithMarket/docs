...
Liquidates an undercollateralized position to maintain protocol solvency. Oracle-gated: disabled when the feed reverts or returns a non-positive price (staleness alone does not disable liquidations).
...
Redeems Coin for collateral from one redeemable borrower at the oracle price minus a redemption fee. The redeemer picks that borrower. There is no shared redeemable pool.

**Parameters:**
- `borrower`: Address of the free-debt position to redeem against
- `amountIn`: Max amount of Coin to redeem (in wei). Clamped to that borrower's debt and collateral.
...
**Requirements:**
- `borrower` must be redeemable (free debt) with collateral and debt
- Redemptions must be allowed (`allowLiquidations` from `getCollateralPrice()`; disabled on feed revert / non-positive price, not on staleness alone)
- Remaining debt after the redeem must be 0 or ≥ `minDebt`
...
Returns the normalized collateral price and safety flags from `getCollateralPrice()`.

**Returns:**
- `price`: Normalized to `36 - collateralDecimals` digits (floored to 1 inside the function to avoid division by zero in consumers)
- `reduceOnly`: True when expanding debt/collateral is blocked (stale oracle, invalid/zero feed price, feed revert, or system undercollateralization)
- `allowLiquidations`: True if liquidations, redemptions, and write-offs are permitted. Set to `false` only when the oracle feed reverts or returns a non-positive price. Staleness sets `reduceOnly` and decays `price` over 24 hours; it does not clear `allowLiquidations`.
...
Permissionlessly mints accumulated local reserves to the instance `operator` (not to `msg.sender`).

**Behavior:**
- Accrues interest and PSM profit, then mints `accruedLocalReserves` of Coin to `operator` and resets the accrual to 0
- Callable by anyone; early-returns if there is nothing to pull
