...
The Vault contract implements ERC4626 tokenized vault functionality for yield farming and staking.
...
- `receiver` cannot be zero address
- If `owner != msg.sender`, caller must have redemption approval
...
**Note:** Includes accrued interest and staking rewards.
...
### Lender Integration
The vault is tightly integrated with the Lender contract:
- Receives staking rewards from lender
- Provides boosted yields for long-term holders
- Shares accrue value from protocol revenue

### ERC4626 Compliance
Fully compliant with ERC4626 standard:
- Atomic deposit/mint operations
- Preview functions for accurate quoting
- Proper rounding and slippage protection
...
- Withdrawals protected against rounding errors
- Maximum operation limits prevent overflow
